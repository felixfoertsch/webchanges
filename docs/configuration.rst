...
Footnote
--------
The top-level ``footnote`` directive is a string (or ``null``) appended to the footer of every report (text, HTML and
markdown). Use it to add a fixed disclaimer, a link, or contact info to all reports.

.. code-block:: yaml

   footnote: "Reports generated by webchanges -- contact admin@example.com"

If set to ``null`` (the default) or omitted, no footnote is added.
...
can see a list of such omitted/missing directives and the default values assigned when running with the ``--vv`` or ``-vvv`` command
line argument.
